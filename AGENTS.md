...
- **Hermes MCP GUI autopilot (Plan 7)**: Hermes can drive the user's mouse and keyboard via three destructive MCP tools — `click_element` (visible glow-cursor flight then HID click), `type_text` (pasteboard staging + Cmd+V, clipboard restored), `press_keyboard_shortcut` ("Cmd+S" / "Return" / etc. → virtual key codes). All three route through `ActionExecutor.shared` so they share the `GUIActionMutex` with voice-mode `WorkflowRunner` and the background-agent swarm — no interleaved HID events. Gated by `CompanionManager.hermesGUIAutopilotEnabled` (default off, persisted under `UserDefaults` key `"hermesGUIAutopilotEnabled"`, exposed in the menu bar panel's Dev section as "Hermes can drive my Mac"). When Plan 6 (Guardrails) ships, this flag is replaced by `GuardrailsDecider` — see Task 8 of `docs/superpowers/plans/2026-05-14-plan-7-hermes-mcp-gui-autopilot.md` for the migration steps.
...
| `TipTour/Hermes/TypeTextMCPTool.swift` | ~90 | MCP tool — `type_text`. Calls `ActionExecutor.shared.typeText` (pasteboard staging + Cmd+V, layout-agnostic, clipboard restored). No cursor flight — typing isn't spatial. Optional `app_hint` resolves to `NSRunningApplication` via bundle-id then localized-name substring. Gated by `hermesGUIAutopilotEnabled`. |
| `TipTour/Hermes/PressKeyboardShortcutMCPTool.swift` | ~80 | MCP tool — `press_keyboard_shortcut`. Passes the shortcut string straight to `ActionExecutor.shared.pressKeyboardShortcut` (which parses "Cmd+S" / "Cmd+Shift+T" / "Return" / etc. and posts virtual key codes at HID level). Same `app_hint` resolution as `TypeTextMCPTool`. Gated by `hermesGUIAutopilotEnabled`. |
