...
- **First-run setup**: A fresh install has no `~/.hermes/config.yaml`. The Dev section of the menu bar panel shows a "Hermes provider" picker (Anthropic / OpenAI / Google) + BYOK rows for each provider's API key. Picking a provider via the picker writes `~/.hermes/config.yaml` via `HermesConfigBootstrapper.writeMinimalConfig` and stops the running Hermes subprocess so the next chat send re-launches with the matching env var. The key rows write into Keychain under `anthropicAPIKey` / `openAIAPIKey` / `geminiAPIKey` (Google's "Gemini" key is shared with the existing `GeminiLiveSession` consumer). `HermesClient.launchSubprocessIfNeeded` reads the chosen provider's keychain key and injects it as `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY` into the subprocess environment. If Hermes is asked to send before either config or the matching key is in place, `HermesClient.send` short-circuits with an actionable system-error transcript entry.
...
| `TipTour/Agents/UI/SettingsView.swift` | ~290 | Three-tab Settings sheet: `AgentsSettingsView` (model routing per task type + token budgets + max-concurrent stepper), `SkillsSettingsView` (browse/delete skill library with Clear All), `LearningSettingsView` (self-critique threshold slider + memory management). Presented from the companion panel footer's Settings button. |
| `TipTour/Hermes/HermesConfigBootstrapper.swift` | ~95 | Writes minimum-viable `~/.hermes/config.yaml` so the bundled runtime can complete `session/new` without `hermes setup`. Provider enum (anthropic / openai / google) carries display name, env var name, Keychain key, and default model — single source of truth for provider strings. Used by `CompanionPanelView`'s Hermes-provider picker and `HermesSetupCoordinator`. |
...
| `TipTour/Hermes/HermesSetupCoordinator.swift` | ~85 | Cross-references `config.yaml` (via `HermesConfigBootstrapper`) against the Keychain (via injected `HermesProviderKeyReader`) to answer "is the runtime ready to take a prompt?". `HermesClient.send` pre-flights through this; `CompanionPanelView.devToolsSection` reads `configuredProvider` through this to seed the Hermes-provider picker. `KeychainProviderKeyReader` is the production implementation; tests inject fakes. |
