...
final class GeminiLiveSession: ObservableObject {
    ...
    /// True once any non-empty user transcript has arrived for the
    /// current session. Used by `handleToolCall` to reject tool calls
    /// fired before the user has actually spoken — Gemini will
    /// otherwise act on the screenshot alone as soon as the session
    /// opens (e.g. flying the cursor to whatever's on screen) which
    /// looks like the app is doing random things on its own.
    /// Resets to false on every fresh session.
    private var hasReceivedUserSpeechThisSession: Bool = false
    ...
    func start(initialScreenshot: Data?) async throws {
        ...
        isActive = true
        inputTranscript = ""
        // Fresh session — Gemini hasn't heard the user yet. Tool calls
        // before the first transcript are dropped by handleToolCall().
    ...
    private func handleGeminiEvent(_ event: GeminiLiveEvent) {
        ...
        case .inputTranscript(let text):
            ...
            // Unlock tool calls only when we have a CONFIDENT speech
            // signal. The prior "any non-whitespace string" check was
            // too loose: Gemini's transcriber sometimes emits a single
            // syllable ("uh", "hm") or punctuation when it picks up
            // ambient noise / breathing / keyboard taps, and that's
            // enough to flip the gate before the user has actually
            // said anything. The model then fires point_at_element on
            // whatever's on screen, which reads to the user as "the
            // app is acting on its own".
            //
            // Heuristic: require at least one *word* of 3+ ASCII letters
            // in the accumulated transcript. Catches real first words
            // like "hey", "yes", "open", while rejecting filler
            // syllables ("uh", "hi") and pure-punctuation transcripts.
            if !hasReceivedUserSpeechThisSession,
               transcriptLooksLikeRealSpeech(inputTranscript) {
                hasReceivedUserSpeechThisSession = true
                // Log the trimmed content so false-positives are
                // visible in the console — if you ever see this fire
                // when you didn't speak, paste the transcript so we
                // can adjust the heuristic.
                let trimmed = inputTranscript.trimmingCharacters(in: .whitespacesAndNewlines)
                print("[GeminiLiveSession] 🎙️ first user speech detected — tool calls now permitted (transcript: \"\(trimmed)\")")
            }
    ...
    // MARK: - Speech detection heuristic

    /// Returns true when `transcript` looks like a genuine spoken
    /// utterance rather than transcriber noise. We require at least
    /// one alphabetic token of length ≥ 3 — this admits real first
    /// words ("hey", "yes", "open", "save") while rejecting:
    ///   - punctuation-only transcripts ("." / "?")
    ///   - 1–2 char filler syllables that Gemini's recogniser
    ///     occasionally emits for breathing, mouse clicks, or keyboard
    ///     taps when the mic is hot but the user hasn't spoken yet
    ///     ("uh", "hm", "a", "I")
    ///   - whitespace-only chunks
    ///
    /// Edge cases this still passes through:
    ///   - The user genuinely says "hi" first → they'll need to follow
    ///     it with a real word for the gate to flip. Acceptable trade
    ///     because the alternative is the agent acting on every cough.
    private func transcriptLooksLikeRealSpeech(_ transcript: String) -> Bool {
        let trimmed = transcript.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return false }
        // Split into "words" along whitespace + punctuation. Then keep
        // the ones with ≥ 3 alphabetic characters.
        let words = trimmed.unicodeScalars.split { scalar in
            !CharacterSet.letters.contains(scalar)
        }
        return words.contains { wordChars in
            wordChars.count >= 3
        }
    }
    ...
    private func handleToolCall(id: String, name: String, args: [String: Any]) {
        ...
        // Hard gate: refuse tool calls that arrive before the user has
        // actually spoken in this session. Gemini will otherwise act on
        // the initial screenshot alone (e.g. fly the cursor to whatever
        // is on screen the moment the session opens) which reads to the
        // user as "the app is doing random things on its own". The
        // system prompt already says don't act before user speech, but
        // a server-side check is the only thing that's actually
        // enforceable.
        if !hasReceivedUserSpeechThisSession {
            print("[GeminiLiveSession] 🚫 dropping tool call \(name) — user has not spoken yet this session")
            let refusal: [String: Any] = [
                "ok": false,
                "error": "no_user_speech_yet",
                "message": "You may not call tools before the user has spoken in this session. Stay silent and wait for the user's first utterance."
            ]
            geminiClient.sendToolResponse(id: id, name: name, response: refusal)
            return
        }
