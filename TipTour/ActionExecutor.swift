...
final class ActionExecutor {
    ...
    func click(
        ...
        try await GUIActionMutex.runExclusive { [self] in
            try await performClickHoldingMutex(
    ...
    private func performClickHoldingMutex(
        atGlobalScreenPoint globalScreenPoint: CGPoint,
    ...
    func pressKeyboardShortcut(
        ...
        try await GUIActionMutex.runExclusive { [self] in
            try await performKeyboardShortcutHoldingMutex(
                shortcutString,
                activatingTargetApp: targetApp
            )
        }
    ...
    private func performKeyboardShortcutHoldingMutex(
        _ shortcutString: String,
        activatingTargetApp targetApp: NSRunningApplication?
    ) async throws {
        try assertAccessibilityPermissionGranted()
    ...
    func typeText(
        ...
        try await GUIActionMutex.runExclusive { [self] in
            // Call the private Cmd+V path directly so we don't try to
            // re-acquire the (non-reentrant) mutex from within itself.
            try await performTypeTextHoldingMutex(text, activatingTargetApp: targetApp)
        }
    }

    private func performTypeTextHoldingMutex(
        _ text: String,
        activatingTargetApp targetApp: NSRunningApplication?
    ) async throws {
        try assertAccessibilityPermissionGranted()
        ...
        // Use the inner Cmd+V path, NOT the public pressKeyboardShortcut,
        // because we already hold the GUIActionMutex and the mutex is
        // non-reentrant.
        try await performKeyboardShortcutHoldingMutex("Cmd+V", activatingTargetApp: nil)
