...
private struct FlexiblePlanPayload: Codable {
    ...
    func toPlan() -> WorkflowPlan? {
        ...
        let normalizedSteps: [WorkflowStep] = rawSteps.enumerated().map { index, s in
            return WorkflowStep(
                ...
                type: Self.normalizeStepType(s.type ?? s.action),
    ...
    // The LLM's step type can arrive in several shapes — strict camelCase
    // from our schema ("keyboardShortcut"), snake_case from a model that
    // assumed Python conventions ("keyboard_shortcut"), or a loose verb
    // ("press", "shortcut"). We try the strict enum raw-value first so the
    // common case is one comparison, then fall back to a case- and
    // separator-insensitive match against known synonyms. Mis-classifying a
    // step as `.click` silently downgrades autopilot to teaching mode, so
    // we are tolerant on input but never lose the original intent.
    private static func normalizeStepType(_ raw: String?) -> WorkflowStep.StepType {
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !trimmed.isEmpty else { return .click }
        if let exact = WorkflowStep.StepType(rawValue: trimmed) {
            return exact
        }
        let collapsed = trimmed.lowercased()
            .replacingOccurrences(of: "_", with: "")
            .replacingOccurrences(of: "-", with: "")
            .replacingOccurrences(of: " ", with: "")
        switch collapsed {
        case "click", "tap", "press", "leftclick":
            return .click
        case "keyboardshortcut", "shortcut", "hotkey", "keybinding", "keypress", "key":
            return .keyboardShortcut
        case "type", "typing", "input", "enter", "text", "typetext":
            return .type
        case "scroll":
            return .scroll
        case "waitforstate", "wait", "waitfor":
            return .waitForState
        case "observe", "highlight", "look":
            return .observe
        default:
            return .click
        }
