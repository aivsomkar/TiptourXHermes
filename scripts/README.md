...
## `validate-bundled-skills.sh` — Spec-check bundled skills

Runs the upstream `skills-ref validate` CLI against every bundled SKILL.md folder under `TipTour/Agents/Skills/BundledSkills/` so we catch drift from the [agentskills.io specification](https://agentskills.io/specification).

```bash
./scripts/validate-bundled-skills.sh
```

Falls back to `npx --yes skills-ref` when `skills-ref` isn't on `PATH`. Requires Node.js installed.
